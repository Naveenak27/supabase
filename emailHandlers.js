...
// Extract emails from CSV buffer
...
// Extract emails from ODS buffer
const extractEmailsFromODS = (buffer) => {
  return new Promise((resolve, reject) => {
    try {
...
const uploadFile = async (req, res) => {
  ...
  try {
    ...
    try {
      if (fileExt === '.csv') {
        emails = await extractEmailsFromCSV(fileBuffer);
      } else if (fileExt === '.ods') {
        emails = await extractEmailsFromODS(fileBuffer);
      } else {
      ...
      return res.status(400).json({ 
        error: `Error extracting emails: ${error.message}`,
        details: error.stack 
      });
    }
    ...
    try {
      ...
      console.error('Error with Supabase operation:', error);
      return res.status(500).json({ 
        error: `Supabase operation error: ${error.message}`,
        details: error.stack 
      });
    }
