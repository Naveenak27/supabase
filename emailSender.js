...
// Helper to build Brevo attachment from a file path (base64)
// Pass originalName to use the user's original filename instead of the timestamped disk name
const fileToBase64Attachment = (attachmentPath, originalName) => {
  try {
    // Read file as base64
    const fileBuffer = fs.readFileSync(attachmentPath);
    const content = fileBuffer.toString('base64');
    // Use the original uploaded filename (e.g. "My Resume.pdf") not the disk name (e.g. "1742476353017-My Resume.pdf")
    const filename = originalName || path.basename(attachmentPath);
...
// Function to send email to a single recipient with optional attachment via Brevo
// originalName: the user's original filename (e.g. "My Resume.pdf") — used as the attachment name in the email
const sendEmail = async (recipient, subject, html, attachmentPath = null, originalName = null) => {
  console.log('\n========== SENDING EMAIL ==========');
  console.log('Recipient:', recipient);
  console.log('Subject:', subject);
  console.log('Has attachment path:', !!attachmentPath);
  ...
  if (attachmentPath) {
    ...
    if (fs.existsSync(attachmentPath)) {
      try {
        ...
        // Create attachment — pass originalName so the recipient sees the real filename
        const attachment = fileToBase64Attachment(attachmentPath, originalName);
        sendSmtpEmail.attachment = [attachment];
...
const sendEmailsToAll = async (req, res) => {
  ...
  return Math.floor(Math.random() * (maxDelay - minDelay + 1)) + minDelay;
    ...
    for (let i = 0; i < emails.length; i++) {
      ...
      try {
        await sendEmail(email, emailSubject, emailContent, resumePath, resumeOriginalName);
        results.success.push(email);
        console.log(`✅ Email sent successfully to: ${email}`);
        await logEmailResult(email, emailSubject, 'success');
      } catch (error) {
  ...
  uploadMiddleware(req, res, async (err) => {
    ...
    try {
      ...
      try {
        ...
        await sendEmail(recipient, subject, content, resumePath, resumeOriginalName);
        await logEmailResult(recipient, subject, 'success');
        console.log(`✅ Email sent successfully to: ${recipient}`);
